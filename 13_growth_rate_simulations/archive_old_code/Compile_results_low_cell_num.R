...
for (i in 1:length(clinical_data[,1])){
	#print(i)
	results <- as.data.frame(read.csv(paste("Optimized_parameters", i, "_low_cell_num.csv", sep="")))
	print(results)
	clinical_data$growth[i] <- results[1,2]
	clinical_data$ratio[i] <- results[1,3]
	clinical_data$mutations[i] <- results[1,4]
}


write.csv(clinical_data, "Compiled_growth_ratio_results_low_cell_num.csv")
