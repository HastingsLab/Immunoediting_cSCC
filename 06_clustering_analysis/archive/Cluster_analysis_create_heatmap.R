...
for (i in 1:length(clinical_data$Tumor_Sample_Barcode)){
  ...
  if (!file.exists(paste("Characteristic_tables/", patient_id, "_all_characteristics.csv", sep=""))){
    ...
    data_temp <- as.data.frame(read.csv(paste("Characteristic_tables/", patient_id, "_all_characteristics_add_EC.csv", sep=""), row.names=1))
...
data_raw$min_kd <- log(data_raw$min_kd+0.01,10)
data_raw$min_stab <- log(data_raw$min_stab+0.01,10)
data_raw$DAI <- log(data$DAI+0.01,10)
...
cluster_colors <- c("1" = "#CC6677", "2" = "#661100", "3" = "#999933", "4" = "#DDCC77", "5" = "#117733", "6" = "#88CCEE", "7" = "#332288")
...
t.test(as.numeric(C_ic_c$value), as.numeric(C_ic_sc$value), paired=TRUE)
t.test(as.numeric(C_is_c$value), as.numeric(C_is_sc$value), paired=TRUE)
t.test(as.numeric(C_ic_sc$value), as.numeric(C_is_sc$value))
t.test(as.numeric(C_ic_c$value), as.numeric(C_is_c$value))
