...
colnames(data) <- c("Peptide", "ID", "gene_id", "expression", "TAP", "Cle",
                    "min_kd", "min_stab", "CCF", "EC", "calis_TCR", "DAI", "Patient", "Immunosuppression")
...
for (i in 1:length(clinical_data$Tumor_Sample_Barcode)){
  ...
  if (!file.exists(paste("Characteristic_tables/", patient_id, "_all_characteristics.csv", sep=""))){
    ...
    data_temp <- as.data.frame(read.csv(paste("Characteristic_tables/", patient_id, "_all_characteristics_add_EC_add_PRIME_add_calis.csv", sep=""), row.names=1))
  
    data_temp <- data_temp[, which(colnames(data_temp) %in% c("Peptide", "ID", "gene_id", "expression", "TAP", "Cle",
                    "min_kd", "min_stab", "min_WT_kd", "CCF", "EC", "calis_TCR"))]
    ...
    data_temp <- data_temp[, which(colnames(data_temp) %in% c("Peptide", "ID", "gene_id", "expression", "TAP", "Cle",
                    "min_kd", "min_stab", "DAI", "CCF","EC","calis_TCR", "DAI"))]
...
#data$calis_TCR <- scale(data$calis_TCR, center=TRUE)
data$calis_TCR <- (data$calis_TCR)*20
...
cluster_colors <- c("1" = "#CC6677", "2" = "#661100", "3" = "#999933", "4" = "#DDCC77", "5" = "#117733", "6" = "#88CCEE", "7" = "#332288", "8" = "black", "9" = "grey")
...
pdf("Expression_v_clusters_add_calis.pdf")
...
pdf("C2_v4_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C3_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C4_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C5_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C6_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C7_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
...
pdf("C8_v3_boxplot_add_calis.pdf")
ggplot(C, aes(x=immune_clonal, y=as.numeric(value))) + geom_boxplot(outlier.shape = NA) + geom_jitter(width=0.05) + theme_minimal()
dev.off()
