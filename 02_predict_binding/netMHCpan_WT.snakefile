...
rule all:
    input:
#        expand(os.path.join(peptide_path, "{sample}_netmhc_WT_all_lengths.xsl"), sample=samples, peptide_path=peptide_path),
        expand(os.path.join(peptide_path, "{sample}_netmhc_WT.xsl"), sample=samples, peptide_path=peptide_path)

rule prepare_input_21: 
	input: 
		os.path.join(peptide_path, "{sample}.somaticvariants_filtered_vep_WT_21.peptides")
	output: 
		os.path.join(peptide_path, "{sample}.somaticvariants_filtered_vep_WT_21.peptides_formatted")
	shell: 
		"""
...
rule netMHCpan_all_lengths:
        input:
                os.path.join(peptide_path, "{sample}.somaticvariants_filtered_vep_WT_21.peptides_formatted")
        output:
                os.path.join(peptide_path, "{sample}_netmhc_WT_all_lengths.xsl")
        params:
