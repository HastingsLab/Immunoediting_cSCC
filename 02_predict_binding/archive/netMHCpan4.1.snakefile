...
rule all:
    input:
        expand(os.path.join(peptide_path, "{sample}_netmhc4.1_all_lengths.xsl"), sample=samples, peptide_path=peptide_path)

rule netMHCpan_all_lengths:
        input:
                os.path.join(peptide_path, "{sample}.somaticvariants_filtered_vep_21.peptides_formatted")
        output:
                os.path.join(peptide_path, "{sample}_netmhc4.1_all_lengths.xsl")
        params:
