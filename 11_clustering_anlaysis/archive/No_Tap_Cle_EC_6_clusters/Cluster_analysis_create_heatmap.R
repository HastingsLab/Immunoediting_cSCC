...
## Setup
set.seed(123)
data_plot <- t(data[,c(4:5,7,10,12)])
...
clinical_data_temp <- clinical_data[match(summary$Patient, clinical_data$ID),]
print(identical(summary$Patient, clinical_data_temp$ID))
summary$immune_infiltration <- clinical_data_temp$immune_infiltration
head(summary)

model <- lm(as.numeric(immune_infiltration) <- as.numeric(C1) + as.numeric(C2) + as.numeric(C3) + as.numeric(C4) + as.numeric(C5) + as.numeric(C6), summary)
summary(model) 
